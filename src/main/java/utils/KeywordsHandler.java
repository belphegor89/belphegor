...
import utils.datageneration.keywords.KeywordManager;
...
public class KeywordsHandler {
    ...
    private static final Path KEYWORDS_FILE_PATH = Paths.get(PreExecutionFiles.METADATA_AND_KEYWORDS_FILES_FOLDER_PATH + "/keywords");

    public static void instantiate() {
        try {
            keywordManager = new KeywordManager(KEYWORDS_FILE_PATH);
        } catch (NullPointerException e) {
            LogManager.getLogger().error("Failed to read keywords file located in the " + KEYWORDS_FILE_PATH.toString(), e);
