...
public class ParametersController {
    ...
    private static boolean isMetadata(String value) {
        Matcher metaDataMatcher = ACCEPTABLE_METADATA_PATTERN.matcher(value);
        ...
        if (possibleMetaDataMatcher.matches()) {
            if (metaDataMatcher.matches()) {
                ...
                LogManager.getLogger().info("Passed \"" + value + "\" parameter looks like a metadata key but it's format is not acceptable." + messageToReport);
                return false;
            }
        }
        return false;
    ...
    private static boolean isKeyword(String value) {
        Matcher keywordMatcher = ACCEPTABLE_KEYWORD_PATTERN.matcher(value);
        ...
        if (possibleKeywordMatcher.matches()) {
            if (keywordMatcher.matches()) {
                ...
                LogManager.getLogger().info("Passed \"" + value + "\" parameter looks like a keyword but it's format is not acceptable." + messageToReport);
                return false;
            }
        }
        return false;
     ...
     */
    private static boolean isExecutionContext(String value) {
        Matcher executionContextMatcher = ACCEPTABLE_EXECUTION_CONTEXT_PATTERN.matcher(value);
        ...
        if (possibleExecutionContextMatcher.matches()) {
            if (executionContextMatcher.matches()) {
                return true;
            } else {
                ...
                LogManager.getLogger().info("Passed \"" + value + "\" parameter looks like a execution context key but it's format is not acceptable." + messageToReport);
                return false;
            }
        }
        return false;
    ...
    public static String checkIfSpecialParameter(String parameter) {
        ...
        } else if (isExecutionContext(parameter)) {
